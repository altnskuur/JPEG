...
for i = 1:numIterRow
    for j = 1:numIterCol
        ...
        dctBlock = dctMat*imageBlock*dctMat';
        entropyImBlock = entropy(imageBlock);
        entropyImBlockMat(i,j) = entropyImBlock;
        AdapQParams(i,j) = -2*exp(entropyImBlockMat(i,j)) + 9.3;
        imageBlockMat(:,:,i,j) = round(dctBlock./(AdapQParams(i,j)*quantizMat));
    end
end
...
for i = 1:numIterRow
    for j = 1:numIterCol
        idctImageBlock = dctMat'*(dqImageBlockMat(:,:,i,j).*(AdapQParams(i,j)))*dctMat;
...
imageJPEG = imread('lenaJPEG.bmp');
subplot(1,3,1), imshow(image)
title("Original Image")
subplot(1,3,2), imshow(imageJPEG)
title("JPEG Image")
subplot(1,3,3), imshow(imageRectonst)
title("Reconstructed Image with adaptiveQ")
imwrite(imageRectonst, 'lenaReconstructed AdaptiveQ.bmp')
